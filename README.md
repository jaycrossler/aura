...
/knowledge/_index.md                 — Auto-generated directory tree and metadata index
/knowledge/build_tree.py             — Indexer, validator, and chapter listening-text exporter
...
/knowledge/scenes/chapter_text/       — Generated plain-text files for audiobook review
...
/visual_profiles/                    — Character, location, creature, faction visual and voice specs
/images/                             — Mirrored folder containing generated production illustrations
/images/_image_manifest.md           — Image generation manifest (prompts, details, status)
/VISUAL_PROFILE_SYSTEM_INSTRUCTIONS.md — Instructions for visual profiling & scene metadata
```

## Working Conventions

- **/knowledge** All knowledge in the knowledge bank is kept within this folder.
- **Index, Tree, and Listening Text Management**: Run `python build_tree.py` inside the `knowledge/` directory whenever you add, delete, or modify knowledge files. The script rebuilds `_index.md`, checks warnings, and refreshes individual and five-chapter plain-text listening files under `scenes/chapter_text/`.
- **Visual Profiles and Image Assets**: Visual details for character and location image prompts live in `/visual_profiles/` and follow the guidelines in `/VISUAL_PROFILE_SYSTEM_INSTRUCTIONS.md`. Images are generated by the Image Generation Agent under `/images/` using mirroring rules and tracked via `/images/_image_manifest.md`.
- **File IDs** use the pattern `category_descriptor` (e.g. `char_jace_apollo`, `tech_aura`, `magic_returning_arts`).
- **YAML frontmatter** at the top of each file holds structured metadata; markdown body holds prose. Every markdown file must have a descriptive `description` field in frontmatter.
