---
id: event_jace_mars_tour_with_rich
title: Mars Sponsor Tour with Lt. Rich Cullivan
type: scene
status: pre-draft (emotional beats / structural sketch)
book: Book 1
chapter: TBD (early chapter — first 2-3)
pov: Jace Grant
date_in_universe: early 2105
last_updated: 2026-04-30
---

# Scene — Mars Sponsor Tour with Lt. Rich Cullivan

## Purpose

Establish Mars as a setting through Jace's eyes. Introduce Lt. Rich Cullivan as a recurring presence and grounded counterpoint. Plant the HUD-cost / fiscal-naïveté beat that pays off catastrophically when Jace is discharged. Show Jace at peak golden-boy confidence so the fall lands harder.

## Position in Sequence

After Jace disembarks the shuttle on Mars; before his medical checkout. Probably interleaved with or adjacent to the drone control center tour. Could be split across two short scenes (initial pickup + later sightseeing) if needed for pacing.

## Setting

Multiple Mars locations as Rich drives Jace around in a pressurized rover:
- The shuttle landing pad and short transit tunnel to base
- The U.S. Mars Detachment perimeter (visible from the rover)
- One of the larger civilian dome districts (windows out to Mars surface, pedestrians, shops, screens displaying Earth news)
- Briefly: a public park area where Mars residents congregate around the few real plants
- Possibly: a glance toward the fabrication district, the big farm domes, or the consumer electronics district as background context

## Characters Present

- Jace Grant (POV — fresh off shuttle, slightly disoriented by Mars gravity, riding high)
- Lt. Rich Cullivan (sponsor — calm, friendly, observant)

## Emotional Beats


### Beat 1: Welcome — Terminal, Biometric Scan (~02:00)

Rich is waiting at the arrivals barrier, easy posture, standard travel mug in hand — the base emblem worn smooth from a year of daily carry. He spots Jace coming through the customs gate and raises the mug in a small greeting.

The terminal is loud and crowded at this hour. The *Pinta* is not the only ship that docked tonight; two cargo cyclers turned their passenger manifests loose in the same window, and the arrivals hall is a controlled crush of bodies. Families reuniting, contractors meeting company reps, a knot of civilian researchers comparing notes, colonists looking dazed in the particular way of people who have just spent four months in a rotating tube and are now being asked to function again. The customs queue shuffles forward in a long S-curve under lighting that is very white and very unforgiving.

Jace is still compensating for gravity. The *Pinta* had run its rotation at almost Mars gravity — *almost* — and the final week of approach had dialed it down incrementally so nobody disembarked in a heap. But "almost" is doing real work in that sentence, and he and the other passengers have spent the week quietly stumbling into bulkheads. He's walking at maybe ninety percent normal confidence. He plays it at a hundred.

He and Rich shake hands — easy, professional, the handshake of two people who have read each other's service records and are now doing the physical version of that. Both in uniform. A few people in the crowd notice. On Mars, American military uniforms draw a glance; people measure the patches, the rank, the posture. Jace is too new to read the looks yet. Rich clocks them all reflexively and steers them slightly to the left, toward a less-trafficked stretch of the barrier wall.

> Rich: "Welcome to Mars, fellow LT."
>
> Jace: "Thanks. You look exactly like your videos."
>
> Rich: "I'm going to pretend that's a compliment." [glances at the crowd, drops his voice one notch — not conspiratorial, just professionally considered] "I need to run your security onboarding before we do anything else. Log every new arrival into the base network. Regulations." [produces the hardened box from under his arm — matte black, reinforced corners, the kind of object that implies a history of being dropped and surviving] "Right hand flat on this."

Jace puts his palm on the surface. The box hums briefly. A faint warmth across his palm — the capillary scanner mapping the vessels beneath the skin.

A green tone.

His MilX Military OS flashes a notification across the lower edge of his overlay:

***Level 2 Access Granted. Local Military Network Established. Welcome to Mars Detachment, Lieutenant Grant.***

He blinks. The overlay populates — base map, local drone traffic summary, a handful of classified feeds grayed out pending command-level clearance. The network is *fast*. After four months on the *Pinta*'s throttled carrier comms, being plugged into a real military network feels like surfacing from underwater.

Rich watches Jace's eyes track the incoming data and allows himself a small grin.

> Rich: "I guess it's really you."
>
> Jace: "What — do you ever have people faking it?"
>
> Rich: [easy, walking them slowly toward the exit] "You hear stories. Identity spoofing's gotten sophisticated. The capillary scan makes it harder." [tucks the box back under his arm] "Anyway. After you formally take command you'll get Level 3 — that's when the drones actually listen to you. For now you've got the full picture, read-only on the command feeds."
>
> Jace: "What's Level 3 like?"
>
> Rich: "Buy me a coffee and I'll tell you." [raises the mug] "Actually — I'll tell you while I drink the coffee I already bought myself. Let's get to the rover."

A contractor family cuts across their path — two adults, three kids who have clearly been aboard a ship for months and have just remembered they have legs. Rich steps back without breaking stride, waits the three seconds for them to clear, resumes walking. The kids are running in small looping arcs, re-learning what gravity feels like. Jace watches them and feels a sudden, specific affection for the human species.

The exit doors open into a pressurized tunnel leading to the vehicle bay. The noise level drops. Rich exhales slightly — the private register of someone who is comfortable outside the crowd.

---

### Beat 2: The Drive — First Look at Mars (~02:30)

The rover is a pressurized six-seater, slightly battered, with a polycarbonate windscreen thick enough to stop a low-velocity micrometeorite. Rich drives. Jace takes the passenger seat and, for the first time, sees Mars without a screen mediating it.

*Red sky.* Not the cinematic red of promotional footage — a washed, dusty, orange-grey sky with the saturation dialed down, like a photograph taken through slightly dirty glass. The horizon is *close*, closer than Earth's; the planet curves away faster than his body expects. Dust moves along the surface in thin sheets. The dome they just left recedes in the wing mirror — enormous from this angle, the polycarbonate glowing warmly against the dark.

People on the perimeter path outside the dome are *tall*. Thin in the specific way of long-term Mars residents: years of 0.38g pull the spine long and redistribute muscle mass upward and outward. He can already identify the new arrivals — shorter, denser, still carrying Earth's gravity in the way they hold their shoulders.

The sky is wrong in a way he can't quite articulate. Rich notices him noticing.

> Rich: "Takes a few days to stop looking at it."
>
> Jace: "It's not what I expected."
>
> Rich: "Everyone says that. What did you expect?"
>
> Jace: "Redder, I think. More dramatic."
>
> Rich: "Give it a couple of hours. When the sun hits the dust at a low angle it goes full dramatic. You'll see." 

3. **The HUD question.** 

>[sip] "How's the overlay? First time on a real network in months?"
>
> Jace: [eyes still scanning the Military OS feeds loading into his peripheral vision] "It's fast. The *Pinta* gave us about 1% of this bandwidth."
>
> Rich: "Yeah, carrier comms are brutal. You're going to love Mars on that front — everything works here. Almost everything."
>
> Jace: "Almost?"
>
> Rich: "Belt's a different story. Latency goes up, reliability goes down. But here? You're golden." [glances over] "Your civilian partition loaded yet?"
>
> Jace: [switches partition in his overlay] "Loading. I think I've got six months of game updates queued. The *Pinta* kept telling me I was 'syncing' and nothing was happening."
>
> Rich: [dry] "Priorities."
>
> Jace: "I have three hundred friends back home waiting for me to come online. My sister's been running raids without me for six months."
>
> Rich: "She been carrying you?"
>
> Jace: "She's been *bragging* about it."
>
> Rich: "Sounds about right."

A long beat. They pass a section of the dome perimeter where a work crew — four humans and a dozen robots — is patching a seam in the outer shell. The robots move in coordinated arcs, their work precise and unhurried. The humans stand slightly apart, monitoring, occasionally redirecting.

> Rich: "You'll get used to the robots fast. They're everywhere. Inside, outside, underground. After a week you stop seeing them."
>
> Jace: "I grew up near a distribution center. It was like that."
>
> Rich: "Mars is more so. Ten robots for every person here, roughly. More in the industrial sections."

Jace watches one of the bots apply sealant in a long smooth pass. His MilX overlay tags it automatically — model number, maintenance status, last inspection date — and he realizes with a small private jolt that this is what Level 2 access actually *means*. Not just a map. Not just comms. The whole environment is annotated now, everything tagged and classified and streaming status data into his peripheral vision.

He is going to like this job.

---

### Continuity Notes for This Scene

- **Rich's coffee:** Standard travel mug throughout. Small, well-used, base emblem. He refills once during this sequence. It is not yet the larger canister it will become.
- **MilX:** Jace's Military OS goes from dormant to Level 2 at the biometric scan. His Civilian OS loads shortly after in the rover. The reader first sees both partitions in action here.
- **Crowd dynamics:** Rich is situationally aware throughout the terminal — steps them left, drops his voice, waits for the contractor family to clear. He has done this before. This is a small but consistent character note: Rich is always reading the room.
- **Level 3 foreshadowing:** *"that's when the drones actually listen to you."* Jace never reaches Level 3. Plant it cleanly and let it sit.


4. **The spending reveal.** Jace mentions, with the cheerful pride of someone who hasn't yet learned what money is for, that he has spent close to a year's wages on HUD upgrades, premium app subscriptions, exclusive courseware, and game packs.

5. **The skeptical pause.** Rich's tone shifts — not unfriendly, just *aware*. "Wow. I've got kids back on Earth. I could never afford something like that."

6. **The plan.** Jace explains his fiscal plan with bright confidence: six-year tour, bonuses pay off the debt and tuition. By the time he's a Captain he can afford a nicer model.

7. **Rich's quiet head-shake.** He doesn't argue. Doesn't lecture. Just glances at Jace, looks back at the road, and lets it sit. The reader sees what Jace doesn't.

Rich's tone shifts gently — not interrogating, just curious in the way an older brother might be:

> Rich: "So what's the plan, then? Just — work? Study?"
> Jace: "Yeah. Work, study, gym. Stay focused. Pay it down."
> Rich: "That's it? No entertainment? Maybe meeting a girl?"
>
> Jace blushes. He doesn't mean to. He looks out the rover window and tries to laugh it off.
>
> Jace: "I, uh — I'll figure that part out later."
>
> Rich files it away. Doesn't push.

8. **The park.** They pass — or briefly stop at — a public area where dozens of people are sitting on benches near a stand of small Earth-imported trees. Rich mentions, half-joking, that people miss plants more than they expect to and workers from Pangu come here to see the green. Jace doesn't quite track the comment.

They pass a small group of workers transferring through the dome — Chinese, judging by uniform patches, though most of the personalities of Mars are visibly multinational at the working level. Their coveralls are stained, slightly oily, and there is a faint smell about them that Jace cannot place.
>
> Jace: [quiet] "Is that — jet fuel?"
> Rich: [glancing] "Drone fuel, probably. They're rotating in from Pangu — the Chinese base. The atmosphere over there is rough. Industrial city. The fuel gets into everything — clothes, skin, the air itself. People say you can taste it after a few weeks. Whole base smells like a refinery."
> Jace: "That sounds awful."
> Rich: "It's where the work is, for them. They make a lot of what we burn." [pause] "Be glad you got Terminus."

9. **Coffee Tree Dome:**

Rich leads Jace through an airlock door into a dome that is noticeably warmer than the others — the temperature shifts about five degrees Celsius up, and the air feels heavier on the skin. Lower atmospheric pressure than typical Mars dome standard (or higher — **[STORYBOT]** confirm with author; the point is *different,* and Jace registers the change immediately in his ears or breathing).

Inside: rows and rows of coffee trees. Real, growing coffee trees. Hundreds of them, in tightly spaced cultivation rows under full-spectrum lighting. The smell is faint coffee-green — leafy, not yet roasted. Workers in light coveralls move among the rows, checking moisture and harvest readiness.

Rich's face changes. He smiles wider than he has all morning.
>
> Rich: "These are my favorite. I bring people here just to show them. Mars coffee — *real* Mars coffee, grown right here. The dome has its own atmospheric mix, different temperature, different pressure. The trees prefer it."
>
> Jace: [looking around, impressed] "It smells amazing."
>
> Rich: [holding up his canister, grinning] "Yeah. This is where it comes from. They sell the harvest at *unbelievable* markup, but if you know one of the cultivators, you can sometimes get a better deal."
>
> Jace: "And do you?"
>
> Rich: "I do."
>
[He sips with deep satisfaction.]

This beat plants:
- Mars coffee as a **prized, locally grown commodity** (already canonical in `location_mars_overview.md` — the timeslip + insomnia + coffee-as-essential thread)
- Rich's coffee canister habit deepened — he buys *good* coffee, has cultivator connections
- A specific dome with **different atmospheric conditions** — sets up the bird dome's even-more-different conditions next
- A small character intimacy — Rich showing Jace his favorite place, a low-stakes act of welcome

10. **The Bird Dome:**

They walk a connecting tunnel and arrive at another airlock — but this one has a fee gate. A small reader. A turnstile. A sign listing the entry fee.
>
> Jace: [reading] "A hundred credits *each*?"
>
> Rich: "Yeah. I know. Trust me, it's worth it. I'll get this one."
>
> Rich pays for both of them — two hundred credits, a small but real expense. Jace makes a mental note (*even Rich pays attention to a 200-credit transaction*) and follows him through.
>
> Inside the bird dome: **the air pressure is higher than any other dome they've been in.** Jace feels it immediately in his ears, his sinuses, the slight resistance against his chest when he breathes in. The dome is much taller than the others — built upward to give birds room to actually fly. Trees, real grass, a small fake lake at the center, full-spectrum lighting tuned to a long Earth day.
>
> Birds everywhere. Songbirds in the canopy. Ducks on the lake. **Geese** at the lake's edge, accompanied by a swarm of about a dozen tiny goslings — a recent hatch, still all yellow fluff.
>
> Rich: "There. *That's* what I wanted you to see. New goslings."
>
> Jace: [genuinely delighted] "How many?"
>
> Rich: "Twelve, last count. Hatched a few days ago. There's a full-time channel that just watches the eggs hatch — somebody's always streaming it. Couple million viewers across Mars and Earth, last I checked. Mars Goose Cam is genuinely a phenomenon."
>
> Jace: [laughing] "You're kidding."
>
> Rich: "I'm not. There's also a duck cam, a finch cam, and a peacock cam, but the goose cam is the one. You're going to see promotional clips for it everywhere. The dome makes a fortune off the streaming rights. That's part of why entry costs what it does — they're protecting the on-cam population from rowdy crowds."
>
> They watch the geese for a few minutes. Jace finds himself, briefly, almost *happy*. He hasn't seen this kind of life in months. It reminds him of Virginia and his parent's horse farm.
>
> Rich: [softly] "Yeah. Mars has its moments."

This beat plants:
- The **bird dome** as a real Mars location with engineering specifics (higher pressure for bird flight, taller dome, fake lake, full-spectrum lighting)
- Mars's **whimsical / humane / not-purely-utilitarian** side — the planet has aesthetic and emotional infrastructure, not just industrial
- The **information environment** at work — Mars Goose Cam as a real cultural phenomenon with millions of viewers (cross-references `tech_information_environment.md` nicely)
- The **expense of entertainment on Mars** — 100 credits per person for a bird dome visit; Jace's reaction confirms his cost-naïveté theme
- Rich as the **kind of friend who pays for the experience** without making a thing of it
- A genuine moment of joy for Jace before the discharge — sets up the loss harder


11. **Closing.** Rich drops Jace at his quarters. "Medical checkout tomorrow morning. After that, you're mine — I'll get you to drone command." Jace thanks him. The door closes. Jace is alone in his room on Mars for the first time. He looks out the window at the Martian sky and thinks about how *good* this is.

## Atmospheric Details to Hit

- **Gravity adjustment.** Jace stumbles a little on the way to the rover. Refuses to acknowledge it. Rich pretends not to see.
- **Background noise.** Constant ventilation hum. Jace will adjust to it within days; right now it is loud.
- **Earth screens.** Displays in public areas showing Earth news, Earth weather, Earth sports. People half-watch them the way Earth-bound people half-watch news from far places.
- **Plant scarcity.** People cluster wherever there are real plants. Jace doesn't yet feel the pull; he will.
- **Aura presence.** Rich's Aura quietly handles directions; Jace's Aura, still settling into Mars latency conditions, occasionally lags or surprises him.
- **HUD tutorial moments.** Jace's HUD overlays an info card on a building they pass; he hasn't asked for it. Slight nuisance. He's still learning what to suppress.

## Lines / Beats to Preserve

The HUD/fiscal exchange should land more or less as drafted in the conversation:
- Rich: "Wow. I've got kids back on Earth — I could never afford something like that."
- Jace: "No problem — I volunteered for the six-year tour on Mars. The bonuses should perfectly pay off all my debts and college tuition."
- Rich: "By then, an even newer model will be out."
- Jace: "Yeah, but I'll be a Captain by then and can afford a nicer model."

(Rewrite as needed for prose voice; preserve the *shape* of the exchange.)

## What This Scene Plants

- **Jace's relationship with money.** Establishes the spending pattern that makes the discharge financially catastrophic.
- **Jace's relationship with the HUD.** Establishes how much he loves it, which makes the deactivation a deeper wound than just "lost equipment."
- **Rich's character.** Recurring background presence; positions him to matter later if needed.
- **Mars as a setting.** First real look at the planet through the protagonist's eyes — gravity, sky, ventilation hum, plant-hunger, screens of Earth.
- **Foreshadowing the discharge.** The reader registers what Jace doesn't: this is going somewhere bad.

## Open Questions

- Whether to split this into two scenes (arrival pickup + later sightseeing) or run it as one extended sequence.
- Whether Rich shows Jace any specific high-value Mars locations (drone command, fabrication district, livestock farms) or saves those for later.
- Whether the gym / flirtation beat happens during this same day or the following day.

## Cross-References

- See [Lieutenant Rich Cullivan](../characters/char_lt_rich_cullivan.md)
- See [Jace Apollo Grant](../characters/char_jace_apollo.md)
- See [Mars (Inhabited World Overview)](../locations/location_mars_overview.md)
- See [Scene — The Doctor's Office](event_jace_doctor_appointment.md) (pays off this scene's setup)
- See [Book 1 — Opening Notes](book01-opening-notes.md) for sequencing

## Revision Notes

- 2026-04-30: Initial draft from voice session — emotional beats and structural sketch, not draft prose.
