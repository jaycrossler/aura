---
id: char_suzi
name: Suzi [surname TBD]
aliases: ["Chief", "Chief Suzi"]
species: human
gender: female
pronouns: she/her
approximate_age: early-to-mid 60s
status: alive
first_appearance: Book 1, Fortuna arrival arc (Helena investigation)
pov_character: false
recurring: true (Fortuna authority figure / investigative ally)
last_updated: 2026-06-17

# Affiliations
role: Fortuna Station chief of security / station police (de facto — small frontier force)
former_affiliations: United States military (retired; long career, hands-on)

# Relationships
allies: [char_jace_apollo (grows to trust him), char_nick_lee (relies on his network data)]
related_characters: [char_helena, char_jace_apollo, char_nick_lee]
related_locations: [location_fortuna_station]

# Physical
appearance_summary: Gray-haired, weathered, compact. Carries herself with the unhurried authority of someone who has handled real situations and feels no need to perform it.
distinguishing_features: Watches people the way investigators do — patiently, without making it a thing.

# Story Function
archetypes: [the honest cop, the retired professional pulled back in, the unexpected ally with authority]
character_arc: stumped investigator of an impossible case → first station authority to take Jace seriously
themes_embodied: [integrity over expedience, the limits of conventional investigation against the unexplained]
---

# Suzi

## Summary

Suzi is Fortuna Station's chief of security — a gray-haired ex-US-military retiree who took the job thinking it would be quiet frontier policing and instead caught the **Helena disappearance** (see [[char_helena]]), an impossible locked-room case with no body, no breach, and no evidence.

She is honest to a fault. Faced with zero physical evidence and a station eager to pin it on one of Helena's boyfriends, she refuses to railroad anyone. She would rather say "I don't know" than close a case dirty. This makes her unpopular with people who want the matter settled and quietly makes her the most trustworthy authority figure on the station.

## Why She Trusts Jace

Jace arrives **after** Helena vanished — he was in transit aboard the *Falcon*, with four months of logged alibi and a station full of witnesses to his absence. In Suzi's words (illustrative register, not final prose):

> "Far as I can tell, you're the only grown man on this rock who couldn't possibly have done it. That makes you the only person I can talk to about it without watching my own back."

A clean alibi is the foundation; what builds the relationship is that Jace's engineer's instinct rejects the easy answer for the same reason hers does — *the story doesn't close.* He doesn't know why the impossibility bothers him (the reader, who watched the *Falcon* incursion, suspects). She doesn't know either. They are two people unwilling to pretend a thing makes sense when it doesn't.

This hands Jace an authority-figure ally and an early, low-key investigative seat — an influence-progression beat that costs him nothing and pays off later when the disappearances are no longer isolated.

## Method and Limits

- Works Helena's case as a missing-person/possible-homicide. Interviews the partners. Pulls hatch logs, movement sensors, and (via Nick) the Aura network record.
- Hits the wall every conventional investigator will hit against an accidental crossing: the evidence describes something that cannot have happened. She files it open. She does not invent a culprit to close it.
- Ex-military: comfortable with chain-of-evidence, uncomfortable with mysticism. She will be one of the last to accept a magical explanation and one of the most valuable allies once she does, because her credibility is hard-earned.

## Independent Goals (Side-Character Rule)

Suzi is not a Jace-support appliance. She is keeping a fragile frontier town safe with almost no resources, managing a population that is one bad rumor away from panic, and protecting the station's social fabric against the corrosive effect of an unsolved disappearance during a high-stakes recruitment push. Helena's case threatens all of that. Her interest in solving it is hers.

## Voice and Manner

- Dry, economical, unintimidated. Doesn't raise her voice; doesn't need to.
- Treats fear as a fact to be managed, not indulged.
- Allergic to people who are certain about things they can't possibly know.

## Open Questions

- Surname and specific military background (branch, role, why she emigrated to the belt).
- Whether she is read into the station's suppressed anomaly history, or is being kept in the dark by Origin management.
- How and when she crosses from "stumped" to "willing to entertain the impossible."
- Her relationship to the (TBD) Station Commander and to Origin's preference for quiet.

## Cross-References

- See [[char_helena]] (the case that introduces her)
- See [[char_jace_apollo]] (the alibi'd outsider she comes to trust)
- See [[char_nick_lee]] (the network data she leans on)
- See [[location_fortuna_station]] (her jurisdiction)
- See [[spec_crossing_mechanics]] (the forensic signature she can't explain)

## Revision Notes

- 2026-06-17: New file. Established Fortuna police chief — ex-US-military retiree, honest, stumped by Helena's impossible case, trusts Jace because his transit alibi clears him. Investigative-ally function and independent goals defined.
