...
- **Skill levels:** tracked as discrete integers (L1–L5+); each level meaningfully
  expands what the skill can do, not just how efficiently. L3 is the expert ceiling
...
- L1: 1% conversion — perceptible only over many hours; barely functional
- L2: 5% conversion — noticeable over a full travel day; where most practitioners settle
- L3: 20% conversion — meaningful acceleration within a single travel period; expert level
- L4: 100% — speed doubles each period; enables multi-light-year travel days; Kael's operational level
- L5+: 200%+ — near-exponential; the half-light-year daily ceiling; pre-Scar master level
...
*Kael's [Long Stepping] is L4 — rare even among Woven scouts. It is her most
developed skill and the one that defines her operational range.*
...
- L2: match moderate rotation; board simple spinning structures
- L3: match station rotation; board spinning ships at the axis; expert level
- L4: match orbital velocities; board moving vehicles at full speed
- L5+: instantaneous frame-matching; no approach phase required
...
- L2: hold position against casual force; disengage cleanly
- L3: hold against a single trained practitioner's grab; expert level
- L4: hold against coordinated force from multiple practitioners
- L5+: maintain vector integrity against Will-combat-level pressure
...
- L2: 3 meters; significant but manageable cost
- L3: 10 meters; usable in field conditions; expert level
- L4: 100 meters; tactical utility; can be chained with recovery
- L5+: kilometer range; near-instantaneous; pre-Scar master level
...
- L2: 30% reduction; meaningful survival extension on longer crossings
- L3: distance limit roughly doubled; significant healing boost; expert level — Kael's operational baseline
- L4: near-full healing from dimensional stress between travel days; the practitioner becomes hard to permanently damage
- L5+: Template stabilization against aging; the longevity path; pre-Scar masters

*Kael's [Self Template] is L3 — her survival baseline after three centuries. Without
it she could not operate in the Scar. She will tell Jace that [Self Template] is the
first skill he must develop. She is correct.*
...
- L1: simple objects — rope, hand tools, fabric; forged single-material items;
  Jace's HUD was accidentally L1 here (unstable, expensive to maintain)
- L2: complex single-material objects; simple alloy assemblies; Kael's bracer is L2
- L3: electronics; multi-component assemblies; expert level
- L4: full vehicles; complex machines with multiple interdependent systems
- L5+: buildings, stations, large-scale infrastructure; Titan-wrought construction philosophy
...
- L2: read emotional state via Template; sense Will reserve levels
- L3: assess skill structure broadly; identify trained vs. untrained; expert level —
  Kael's diagnostic of Jace is L3 work
- L4: read individual skills, Will capacity, dimensional history of an object
- L5+: read deep Template history; perceive stored memories in Echo-rich Templates
...
- L2: recognizable objects; simple tools without moving parts
- L3: functional tools and simple machines; expert level
- L4: complex machines; multi-component assemblies; working mechanisms
- L5+: living constructs; bound organisms shaped to Template; the pre-Scar Golem
...
- L2: multi-condition logic; simple permission structures
- L3: conditional logic; "permit entry if Template matches stored pattern"; expert level
- L4: autonomous Will-programs; self-maintaining seals; the Ward Kael teaches at
  Métis Station is high L3–L4
- L5+: self-modifying rune systems; runes that learn and adapt; pre-Scar infrastructure level
...
- L2: blocks moderate fauna; simple keying to one or two Templates
- L3: blocks Tier 2–3 fauna; keyed to a small number of Templates; expert level
- L4: active wards that respond to intrusion attempts; can expel rather than merely
  block; alert the setter when triggered; the Métis Station ward Kael demonstrates
  is high L3–L4
...
- L2: meaningful push; stagger someone; break a grip
- L3: stagger a person forcefully; break loose objects free; force a door; expert level
- L4: structural damage to equipment; break welds; collapse frames
- L5+: tactical demolition; vector collapse on fast-moving projectiles
...
- L2: hold against moderate physical impact; crude pressure reduction
- L3: hold against strong physical impact; partial pressure seal; expert level
- L4: hold atmosphere against hard vacuum for short durations; the air bubble
  Kael extends around Jace during Hyperspace transit is L4
- L5+: functional armor against Will-combat-level attacks; Kael's full layered shells
...
- L2: arrest a running person; stop small thrown objects cleanly
- L3: stop a person's directed movement; freeze small vehicles; expert level
- L4: halt a larger vehicle; freeze multiple targets simultaneously
- L5+: the full stop — drop anything into absolute stillness; what "stopping bullets"
  actually requires
...
- L2: read load distribution across a structure; identify stress concentrations
- L3: map stresses across a structure in real time; predict failure points; expert level
- L4: sense force vectors at distance; predict structural behavior under novel loads
- L5+: read incoming attacks before contact; sense force at planetary scale
...
- L2: meaningfully reduce temperature in a confined space; slow a fire
- L3: freeze a pipe; create localized cold; extract heat from a confined space; expert level
- L4: localized cold zones sustained at range; significant thermal extraction
- L5+: heat extraction from living targets (dangerous, ethically complex)
...
- L2: direct flame direction and rough intensity; functional combat use;
  Jin's level at Arc 1 end — reliable, significant, not yet expert
- L3: shape a burn precisely; sustained without conventional fuel for useful durations;
  expert level; Jin's projected Arc 2–3 level with training
- L4: complex flame geometries; sustained without fuel indefinitely; Bloom-defeating capability
- L5+: plasma-temperature working; fire as a precision tool at distance
...
- L1: slow heat loss; extend survivable time in cold; Jin's passive level at Arc 1 end
- L2: maintain near-comfortable temperature in moderate Astral cold
- L3: maintain comfortable body temperature in Astral cold indefinitely; expert level;
  Kael's operational baseline for Scar travel
- L4: function in extreme thermal environments; extend protection to a companion
- L5+: full thermal control at range; Hyperspace-grade thermal management
...
- L2: read gross features of physical-space objects nearby; Sophia's working level
- L3: read physical-space objects at kilometer range via Astral projection; expert level;
  Kael's operational baseline for scouting
- L4: telescope-grade resolution across the belt; identify ships by hull configuration
- L5+: real-time awareness across an entire station; Odin's ravens

*At L3+, this becomes humanity's most powerful prospecting and intelligence tool.*
...
- L1: sense the presence of the Astral; feel dimensional activity nearby;
  Sophia developed this as a safer alternative to full crossings
- L2: observe gross Astral events from physical space; detect fauna presence
- L3: observe Astral events clearly from physical space; see fauna without being
  visible to them; expert level
- L4: read Astral terrain topology without crossing; navigate by sight from outside
- L5+: perceive Hyperspace from the Astral without entering
...
- L2: sense recent events; feel the shape of what happened without full detail
- L3: reconstruct recent events from a location; see the last 24–48 hours; expert level
- L4: read deep history; access the collapse-layer residue of the Scar
- L5+: commune with coherent Echoes; receive intentional transmissions from stored
  Template fragments
...
- L1: passive (all Sol humans have this from birth); sense Resonance attempts;
  resist casual compulsion; Kael's dominance working finds this at L1 and cannot proceed
- L2: active awareness of Template-reading attempts; resist moderate directed Resonance
- L3: resist strong directed Resonance; actively sense and track intrusion attempts;
  expert level; Kael's active [Mind Wall] is L3
- L4: actively expel an attaching Tick-Maw or Memory Leech; block high-level Resonance
- L5+: complete Template opacity; cannot be read or influenced regardless of attacker's level
...
- L1: hold 10% more than natural baseline; better than radiating freely
- L2: hold 50% more; meaningful tactical reserve
- L3: hold double natural reserve; efficient compression; expert level
- L4: hold a week's generation in reserve; deploy instantly under pressure
- L5+: the "gem-like" quality of Kael's reserves — compressed, contained, waiting;
  her [Will Reservoir] is L4–L5; centuries of discipline
...
- L2: maintain coherence through short Scar transits
- L3: maintain identity coherence through days in the Scar interior; expert level;
  Kael's baseline for operational Scar travel
- L4: operate in high-Echo environments without fragmentation; Scar corridor travel
- L5+: withstand direct Template-to-Template identity attacks; the final line
...
- L2: noticeable unease; most people want to leave the area
- L3: genuine fear response in unshielded practitioners; animals flee; expert level
- L4: Template-shock to unshielded practitioners; the battlefield aura of legendary warriors
- L5+: area suppression; every conscious being in range simultaneously

*Parallel: the divine terror (Greek: deimos/phobos) that demigods project in battle.
The Homeric heroes who caused armies to break were doing this at L4–L5. The Berserker
war-shout is L3–L4 with no theoretical framework behind it.*
...
- L2: hide active skill use; reduce apparent reserve levels
- L3: appear untrained to casual observation; expert level; hide skill structure
  from moderate [Template Reading] attempts
- L4: full Template concealment; appear as an untrained primitive to skilled readers;
  Kael runs this at L4 as baseline, always — it is why Jace cannot read her until
  she deliberately drops it at Métis
- L5+: active misdirection; project a false Template to mislead even expert readers

*Kael drops her [Occlusion] (L4) for Jace after the oath at Métis — one of the few
times he sees her without it. The demonstration (see Scene A-15 in arc_02) is a trust
signal that she would not extend to almost anyone.*
...
- Equipment: the HUD has an unexpected Template on it (L1, accidental, unstable). She
  notes this. She doesn't comment yet.
- [Astral Transfer]: she does not check this on first reading — she does not expect
  a Sol human to have it at all. When she tests it in Arc 2, she finds L3. This is
  the moment she recalibrates.
...
**Kael seen from the Astral (with [Occlusion] L4 dropped):**
...
- 2026-06-27: Level scale revised throughout. L1–L15+ renormalized to L1–L5+ per
  `magic_skill_level_scale.md`. L3 = expert ceiling most practitioners ever reach.
  L4 = rare (Kael's [Long Stepping]). L5+ = legendary/pre-Scar master level.
  All skill trees updated: old L5 → L3, old L10 → L4, old L15+ → L5+. Character
  references updated: Kael's [Long Stepping] L4, [Occlusion] L4, [Force Shell] L4,
  [Self Template] L3, [Psychic Anchor] L3, [Far Sight] L3; Jace's HUD L1 accidental;
  Jin's [Fire Shaping] L2 at Arc 1 end, projected L3 with training.
  Cross-reference to magic_skill_level_scale.md added.