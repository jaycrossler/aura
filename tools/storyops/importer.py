...
def import_from_path(source:Path,knowledge_root:Path)->ImportResult:
    r=ImportResult();processed=[]
    for md in all_markdown_files(source):
        errs_before=len(r.errors)
        for i,(meta,body) in enumerate(parse_frontmatter_chunks(md.read_text(encoding='utf-8'))):
...
def main()->None:
    ...
    ap.add_argument('source');ap.add_argument('--knowledge-root',default='knowledge');ap.add_argument('--report',default='generated/reports/import-report.json')
    ...
    if source.suffix.lower()=='.zip':
        with TemporaryDirectory() as td:
            ...
            result=import_from_path(Path(td),k)
            if not result.errors: source.unlink(missing_ok=True)
    else:result=import_from_path(source,k)
